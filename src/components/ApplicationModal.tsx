...
const ApplicationModal: React.FC<ApplicationModalProps> = ({ isOpen, onClose, product, productType, onNavigate }) => {
  ...
  const validateRefer = (refer: string): string | undefined => {
    if (!refer.trim()) return undefined;
    if (!/^[a-zA-Z0-9_]{3,15}$/.test(refer)) return "Invalid refer code";
    return undefined;
  };
  ...
  const validateField = (field: keyof FormData, value: string): string | undefined => {
    switch (field) {
      ...
      case 'refer': return validateRefer(value);
  ...
  const validateAll = (): boolean => {
    const newErrors: FormErrors = {
      ...
      refer: validateRefer(formData.refer),
      monthlyIncome: productType === 'loan' ? validateIncome(formData.monthlyIncome) : undefined
    };

    setErrors(newErrors);
    setTouched({ fullName: true, mobile: true, email: true, employmentType: true, monthlyIncome: true, date: true, refer: true });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://credgrow.in/api/send-email', {
        method: 'POST',
        headers: {
      ...
      setTimeout(() => {
        if (product?.applyUrl && ('cardName' in product)) {
          ...
          onClose();
          setFormData({ fullName: '', mobile: '', email: '', employmentType: '', monthlyIncome: '', date: '', refer: '' });
          setTouched({});
          setErrors({});
          setSubmitStatus('idle');
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              {/* Refer Code */}
              <div>
                <Label htmlFor="refer" className="flex items-center gap-2 mb-2">
                  Refer By
                </Label>
                ...
                <div className="relative">
                  <Input
                    ...
                    className={`${errors.refer && touched.refer ? 'border-destructive' : ''} pr-10`}
                    placeholder="Enter referral username"
                    aria-invalid={!!errors.refer}
                    aria-describedby={errors.refer ? 'refer-error' : undefined}
                  />

                  {touched.refer && !errors.refer && (
                    <CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-success" aria-hidden="true" />  // ✅ Already correct
                  )}
                  ...
                  {errors.refer && touched.refer && (
                    <p
                      ...
                      role="alert"
                    >
                      <AlertCircle className="w-3 h-3" aria-hidden="true" />  // ✅ Fixed
                      {errors.refer}
                    </p>
                  )}
